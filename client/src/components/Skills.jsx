          ...
          {skills.map((group) => {
            ...
            return (
              <div key={group.category}
                className="bg-white rounded-xl border border-[#e2e8f0] overflow-hidden hover:shadow-md hover:border-[#c9a84c] transition-all duration-200">
                <div className={`${style.bg} px-5 py-4 flex items-center gap-3`}>
                  <div className="text-white opacity-90">{categoryIcons[group.category]}</div>
                  <h3 className="text-sm font-semibold text-white leading-tight">{group.category}</h3>
                </div>
